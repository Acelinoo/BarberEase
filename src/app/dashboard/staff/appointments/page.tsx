...
export default async function StaffAppointmentsPage() {
  ...
  const appointments = await getAppointments({ staffId: session?.user.id });

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Jadwal Saya</h1>
        <p className="text-muted-foreground mt-1">Daftar appointment Anda</p>
      </div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y border-b">
                ...
                {appointments.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-6 py-8 text-center text-muted-foreground">Tidak ada appointment</td>
                  </tr>
                )}
              </tbody>
            </table>
